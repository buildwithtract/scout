import {
  deleteAllExtNged11kvOverheadLines,
  upsertExtNged11kvOverheadLine
} from '@/db/generated/ext_nged_11kv_overhead_lines_sql'
import { Client } from 'pg'
import { ExtOverpass, OverpassFeature } from './ext_overpass'

export class ExtNged11kvOverheadLines extends ExtOverpass {
  name = 'ext-nged-11kv-overhead-lines'
  protected voltage = 11000
  protected voltageTags = ['11000', '11', '11kV', '11000V']
  ...
  async fetch(): Promise<void> {
    console.info('Fetching NGED 11kV overhead lines from OpenStreetMap')
  ...
  private async processFeature(feature: OverpassFeature): Promise<string> {
    try {
      ...
      const result = await upsertExtNged11kvOverheadLine(this.dbClient, {
        uuid: crypto.randomUUID(),
        voltage: voltage,
        situation: situation,
        geometry: geojsonString,
        geometry_3857: geojsonString, // Will be transformed by trigger
        geometry_27700: geojsonString, // Will be transformed by trigger
  ...
  async truncate(): Promise<void> {
    await deleteAllExtNged11kvOverheadLines(this.dbClient)
  }
}
