import {
  deleteAllExtEnw33kvOverheadLines,
  getLatestImportForExtEnw33kvOverheadLines,
  upsertExtEnw33kvOverheadLine
} from '@/db/generated/ext_enw_33kv_overhead_lines_sql'
import { Feature } from 'geojson'
import { Client } from 'pg'
import { OpendatasoftFetcher } from './ext_opendatasoft'

export class ExtEnw33kvOverheadLines extends OpendatasoftFetcher<any> {
  name = 'ext-enw-33kv-overhead-lines'
  ...
  constructor(dbClient: Client) {
    super(
      ...
      'enwl-33kv-overhead-conductors'
    )
  }

  protected async getLatestImportDate(): Promise<Date> {
    const row = await getLatestImportForExtEnw33kvOverheadLines(this.dbClient)
    if (row?.max) {
  ...
  async insertFeature(feature: Feature): Promise<'inserted' | 'updated'> {
    ...
    const result = await upsertExtEnw33kvOverheadLine(this.dbClient, {
  ...
  async truncate(): Promise<void> {
    console.info('Truncating ENW 33kV overhead lines data')
    await deleteAllExtEnw33kvOverheadLines(this.dbClient)
  }
}
