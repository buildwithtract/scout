import {
  deleteAllExtNged33kvOverheadLines,
  upsertExtNged33kvOverheadLine
} from '@/db/generated/ext_nged_33kv_overhead_lines_sql'
import { Client } from 'pg'
import { ExtOverpass, OverpassFeature } from './ext_overpass'

export class ExtNged33kvOverheadLines extends ExtOverpass {
  name = 'ext-nged-33kv-overhead-lines'
  protected voltage = 33000
  protected voltageTags = ['33000', '33', '33kV', '33000V']
  ...
  async fetch(): Promise<void> {
    console.info('Fetching NGED 33kV overhead lines from OpenStreetMap')
  ...
  private async processFeature(feature: OverpassFeature): Promise<string> {
    try {
      ...
      const result = await upsertExtNged33kvOverheadLine(this.dbClient, {
  ...
  async truncate(): Promise<void> {
    await deleteAllExtNged33kvOverheadLines(this.dbClient)
  }
}
