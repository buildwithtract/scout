import {
  truncateExtNged132kvOverheadLines,
  upsertExtNged132kvOverheadLine
} from '@/db/generated/ext_nged_132kv_overhead_lines_sql'
import { Client } from 'pg'
import { ExtOverpass, OverpassFeature } from './ext_overpass'

export class ExtNged132kvOverheadLines extends ExtOverpass {
  name = 'ext-nged-132kv-overhead-lines'
  protected voltage = 132000
  protected voltageTags = ['132000', '132', '132kV', '132000V']
  ...
  async fetch(): Promise<void> {
    console.info('Fetching NGED 132kV overhead lines from OpenStreetMap')
  ...
  private async processFeature(feature: OverpassFeature): Promise<string> {
    try {
      ...
      const result = await upsertExtNged132kvOverheadLine(this.dbClient, {
        voltage: voltage,
        situation: situation,
  ...
  async truncate(): Promise<void> {
    await truncateExtNged132kvOverheadLines(this.dbClient)
  }
}
