...
export const getExtDatagovukScheduledMonumentIntersectingGeometryQuery = `-- name: GetExtDatagovukScheduledMonumentIntersectingGeometry :many
SELECT
    uuid, name, reference, entry_date, start_date, end_date, entity, documentation_url, geometry, geometry_3857, geometry_27700, first_imported_at, last_imported_at
FROM
    public.ext_datagovuk_scheduled_monuments
...
export interface GetExtDatagovukScheduledMonumentIntersectingGeometryArgs {
    geometry: string;
}

export interface GetExtDatagovukScheduledMonumentIntersectingGeometryRow {
    uuid: string;
    name: string;
    reference: string;
    entryDate: Date;
...
export async function getExtDatagovukScheduledMonumentIntersectingGeometry(client: Client, args: GetExtDatagovukScheduledMonumentIntersectingGeometryArgs): Promise<GetExtDatagovukScheduledMonumentIntersectingGeometryRow[]> {
    const result = await client.query({
        text: getExtDatagovukScheduledMonumentIntersectingGeometryQuery,
        values: [args.geometry],
        rowMode: "array"
    });
    return result.rows.map(row => {
